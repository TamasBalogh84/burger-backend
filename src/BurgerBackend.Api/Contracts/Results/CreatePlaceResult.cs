...
public class CreatePlaceResult : ResultBase
{
    ...
    public static CreatePlaceResult Ok(Guid? response) => new(new OkObjectResult(response));
    ...
    /// </summary>
    public static implicit operator CreatePlaceResult(Guid response) => Ok(response);
}